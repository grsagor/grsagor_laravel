<div class="modal-dialog" role="document">
    <div class="modal-content">
        <div class="modal-header">
            <h5 class="modal-title" id="crud_modalLabel">Edit Review</h5>
        ...
        <div class="modal-body">
            ...
            <form id="crud_form" action="{{ route('admin.reviews.update', $review->id) }}" method="POST">
                @csrf
                @method('PUT')
                ...
                <div class="form-group">
                    ...
                    <select class="form-control" id="reviewer_id" name="reviewer_id" required>
                        @foreach($clients as $client)
                            <option value="{{ $client->id }}" {{ $review->reviewer_id == $client->id ? 'selected' : '' }}>
                                {{ $client->name }}
                            </option>
                        @endforeach
                    </select>
        ...
        <div class="modal-footer">
            ...
            <button type="submit" id="crud_form_submit_btn" class="btn btn-primary">Update</button>
        </div>
    </div>
</div>
