...
@section('content')
    <div class="container-fluid px-4">
        ...
        <div class="card my-2">
            <div class="card-body">
                <form action="{{ route('admin.role.update', $role->id) }}" method="post" id="permission_form">
                    @csrf
                    <div class="row">
                        <div class="col-lg-12 form-group">
                            <label for="">Role Name</label>
                            <input type="text" class="form-control" name="" value="{{ $role->name }}" readonly>
                        </div>
                    </div>
                                            ...
                                            <td>
                                                @foreach($right->rights as $row)
                                                    <label for="" class="role-label">
                                                        <input type="checkbox" class="permission-check" name="permission[{{$row->module}}][{{$row->id}}]" @if(Helper::hasRight($row->name, $role->id)) checked @endif value="1">
                                                        {{ explode('.', $row->name)[1] }} 
                                                    </label>
                                                @endforeach
                                            </td>
